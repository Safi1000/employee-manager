...
// Billing month of an invoice: the month its period ENDS in — invoices.service_month,
// generated by the database (0436). 25 Sep – 24 Oct is October. Read, never worked
// out here, so the screen cannot disagree with the ledger about which month an
// invoice belongs to.
const invoiceMonth = (inv: Invoice) => (inv.service_month ?? inv.period_start ?? inv.invoice_date ?? "").slice(0, 7);
...
export default function InvoiceGenerate({ onPosted }: { onPosted: () => void }) {
  ...
  // not the calendar. For a calendar contract it is the calendar month; for a
  // 25th-cycle contract the period ending in this month is 25th–24th, and in
  // the month it switched it is 1st–24th. The database refuses an invoice whose
  // period is not one the contract derives, so drafting the wrong one here would
  // only be refused later — this reads the right one.
  const [billingPeriods, setBillingPeriods] = useState<Map<string, { start: string; end: string }>>(new Map());
  useEffect(() => {
    let alive = true;
    (async () => {
      const { data } = await supabase.rpc("billing_periods_for_month", { p_month: `${period}-01` });
      if (!alive) return;
      const m = new Map<string, { start: string; end: string }>();
      for (const r of (data ?? []) as { contract_id: string; period_start: string; period_end: string }[]) {
        // A contract can have two periods ending in one month (a cycle reverting
        // to the calendar). The later one is drafted; the earlier is billed by hand.
        const cur = m.get(r.contract_id);
        if (!cur || r.period_start > cur.start) m.set(r.contract_id, { start: r.period_start, end: r.period_end });
      }
      setBillingPeriods(m);
    })();
    return () => { alive = false; };
  ...
  useEffect(() => {
    ...
    (async () => {
      const entries = await Promise.all(
        groupClients.map(async (c) => {
          // The client's billing period this month, off any of its contracts;
          // the calendar month only when none derives one.
          const con = contracts.find((k) => k.client_id === c.id && billingPeriods.has(k.id));
          const { start, end } = (con && billingPeriods.get(con.id)) || monthBounds(period);
    ...
    return () => { alive = false; };
  }, [group, period, groupClients, contracts, billingPeriods]);
  ...
  const buildDraft = useCallback(
    (client: Client, con: Contract, taken: Set<string>): Draft | null => {
      const { start, end } = billingPeriods.get(con.id) ?? monthBounds(period);
    ...
    [period, lines, addendums, invoices, suggestNumber, contractPreviousBalance, group, attQty, billingPeriods],
  );
