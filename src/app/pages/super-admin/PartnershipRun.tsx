...
export default function PartnershipRun() {
   ...
   *  and this shows the net and both components so nobody subtracts by hand. */
  const [positions, setPositions] = useState<
    Map<string, { agency: number; remuneration: number; balance: number }>
  >(new Map());
  ...
  const load = useCallback(async () => {
    ...
    // Each active partner's standing position. One call per partner because
    // partner_ledger takes a partner; it is a handful of rows, and the
    // alternative is recomputing a balance the ledger already keeps.
    const { data: pts } = await supabase
      .from("partners").select("id").eq("company_id", companyId).eq("is_active", true);
    const pos = new Map<string, { agency: number; remuneration: number; balance: number }>();
    await Promise.all(((pts ?? []) as { id: string }[]).map(async (pt) => {
      const { data } = await supabase.rpc("partner_ledger", {
        p_partner_id: pt.id, p_start: null, p_end: null,
      });
  ...
  // RESTORED IN THE MERGE. The brief asked for each partner's share with agency
  // and profit shown separately BENEATH ONE NET FIGURE, and this screen was
  // rebuilt in a parallel session without it. Read from partner_ledger, never
  // recomputed from partner_account_entries: those four hand-entered types are
  // only half a partner's current account and omit every agency movement, which
  // is the defect the Partners Summary tab already had once.
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-xs text-slate-500 uppercase">
                  <tr>
                    ...
                    <th className="text-right px-4 py-2">Net position</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {partnerRows.map((r, i) => (
                    <tr key={`${r.partner_id ?? i}`}>
                      ...
                      {/* One net figure, with its two components under it. A
                          partner "owed nothing" may be holding 80,000 of company
                          cash against 80,000 of profit share, and the net alone
                          hides that. */}
                      <td className="px-4 py-2 text-right">
                        <div className="tabular-nums text-slate-900">
                          {acct(positions.get(r.partner_id ?? "")?.balance ?? 0)}
                        </div>
                        <div className="text-[11px] text-slate-500 tabular-nums">
                          profit {acct(positions.get(r.partner_id ?? "")?.remuneration ?? 0)}
                          {" · agency "}
                          {acct(-(positions.get(r.partner_id ?? "")?.agency ?? 0))}
                        </div>
                      </td>
                    </tr>
                  ))}
