...
export default function Clients() {
  ...
  const computeClientErrors = (f: ClientForm): Record<string, string | null> => {
    const errs: Record<string, string | null> = {
      ...
      // Both required: the prefix drives invoice numbers and the client-scoped
      // guard code, and industry is relied on for reporting breakdowns.
      employee_id_prefix:
        f.employee_id_prefix.trim() === ""
          ? "Client prefix is required."
          : validateEmployeeIdPrefix(f.employee_id_prefix),
          ...
          <div>
            ...
            <ThemedSelect
              ...
              onChange={(e) => {
                ...
                validateField("industry", e.target.value ? null : "Industry is required.");
              }}
              className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm"
            >
            ...
            {fieldErrors.industry && <p className="text-xs text-danger-600 mt-1">{fieldErrors.industry}</p>}
          </div>
          <div>
            ...
            <input
              ...
              onBlur={(e) =>
                validateField(
                  ...
                  e.target.value.trim() === ""
                    ? "Client prefix is required."
                    : validateEmployeeIdPrefix(e.target.value),
                )
              }
              maxLength={6}
