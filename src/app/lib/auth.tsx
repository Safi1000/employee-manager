  ...
  const loadProfileAndCompany = async (userId: string): Promise<boolean> => {
    try {
      const { data } = await supabase
        .from("profiles")
        .select("id, company_id, branch_id, role, title, full_name, email, avatar_url, display_company_name, view_as_company, permissions, must_change_password, created_at, updated_at")
        .eq("id", userId)
        .maybeSingle();
