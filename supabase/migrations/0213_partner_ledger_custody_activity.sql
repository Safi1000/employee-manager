-- 0213 — partner_ledger() now also lists every cash movement the partner handles
-- as a CUSTODIAN (a Banks & Ledgers account whose custodian is this partner):
-- client cash received, expenses paid, cash cheques, custody transfers, and
-- bank→cash withdrawals/payroll. These are cash IN/OUT of the partner's hands, so
-- they move the running balance the way drawings/contributions do (cash the
-- partner holds goes up on money in, down on money out). Capital entries,
-- remuneration and GL activity on the partner's equity account are unchanged.
--
-- Sign: balance += remuneration - cash_paid, so cash the partner RECEIVES is a
-- negative cash_paid (balance up = they hold more) and cash they PAY OUT is a
-- positive cash_paid (balance down).
...
begin
  ...
  return query
  with ploc as (
    -- the partner's custodian cash location(s)
  ...
  custody as (
    -- Client cash received into the partner's custody (cash IN → balance up)
    select ip.payment_date::date, 'CLIENT CASH — ' || coalesce(c.name, 'client'),
      ...
      join ploc on ploc.id = ip.custodian_location_id
      left join public.clients c on c.id = ip.client_id
     where ip.payment_date between v_from and v_to
    union all
    -- Expenses paid from the partner's custody (cash OUT → balance down)
    select e.expense_date::date, 'EXPENSE — ' || coalesce(nullif(e.description,''), 'expense'),
      ...
      join ploc on ploc.id = e.custodian_location_id
     where e.expense_date between v_from and v_to
    union all
    -- Cash advances paid from custody (cash OUT)
    select a.advance_date::date, 'ADVANCE',
           a.amount, 0::numeric, 'CUSTODY:ADVANCE'::text, null::uuid
      from public.advances a
      join ploc on ploc.id = a.custodian_location_id
     where a.advance_date between v_from and v_to
    union all
    -- Cleared cash cheques handed to the partner (cash IN)
    select ch.cheque_date::date, 'CHEQUE #' || coalesce(ch.cheque_number,''),
      ...
      join ploc on ploc.id = ch.custodian_location_id
     where ch.cheque_type = 'cash' and ch.status = 'cleared'
       and ch.cheque_date between v_from and v_to
    union all
    -- Custody transfers IN (someone → partner)
    select t.date::date, 'TRANSFER IN',
           -t.amount, 0::numeric, 'CUSTODY:TRANSFER_IN'::text, null::uuid
      from public.custody_transfers t
      join ploc on ploc.id = t.to_location_id
     where t.date between v_from and v_to
    union all
    -- Custody transfers OUT (partner → someone)
    select t.date::date, 'TRANSFER OUT',
           t.amount, 0::numeric, 'CUSTODY:TRANSFER_OUT'::text, null::uuid
      from public.custody_transfers t
      join ploc on ploc.id = t.from_location_id
     where t.date between v_from and v_to
    union all
    -- Bank → partner cash (withdrawals, payroll); cash_delta is signed (+in/-out)
    select bt.created_at::date, coalesce(nullif(bt.description,''), 'Bank / cash movement'),
      ...
      join ploc on ploc.id::text = bt.reference_id
     where bt.kind in ('withdraw_to_cash','payroll')
       and bt.created_at::date between v_from and v_to
  ),
  merged as (
