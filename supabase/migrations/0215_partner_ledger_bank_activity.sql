-- 0215 — partner_ledger() also lists the full statement of any BANK ACCOUNT the
-- partner OWNS (bank_accounts.owner_partner_id). Every bank movement — client
-- receipts, expenses paid, transfers, opening — reads from bank_transactions,
-- where account_delta is the signed effect on that account. Money INTO the
-- partner's account (positive delta) is a negative cash_paid (balance up); money
-- OUT (negative delta) is a positive cash_paid.
--
-- This is additive to 0213/0214: cash custody keys on cash_locations
-- (custodian_partner_id) while this keys on bank_accounts (owner_partner_id), so
...
declare
  v_opening numeric; v_from date; v_to date; v_coa uuid; v_company uuid;
begin
  ...
  custody as (
    ...
    select t.date::date, 'TRANSFER IN', -t.amount, 0::numeric, 'CUSTODY:TRANSFER_IN'::text, null::uuid
      from public.custody_transfers t join ploc on ploc.id = t.to_location_id
    ...
    select t.date::date, 'TRANSFER OUT', t.amount, 0::numeric, 'CUSTODY:TRANSFER_OUT'::text, null::uuid
      from public.custody_transfers t join ploc on ploc.id = t.from_location_id
     where t.date between v_from and v_to
    union all
  ...
  pbank as (
    -- Full statement of a bank account the partner OWNS.
    select bt.created_at::date,
           upper(bt.kind)
             || case when bt.description is null or bt.description = '' then '' else ' — ' || bt.description end
             || ' (' || ba.bank_name || ')',
           -bt.account_delta, 0::numeric, 'BANK:' || upper(bt.kind), null::uuid
      from public.bank_accounts ba
      join public.bank_transactions bt on bt.bank_account_id = ba.id
