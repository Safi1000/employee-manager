-- 0218 — fix statement timeout on the partner ledger. Showing "all dates" made
-- the caller pass p_start=2000 / p_end=2999; the monthly remuneration loop then
-- generated ~12,000 months and called the (heavy) partnership_allocation() once
-- per month. Bound the months series to the partner's real active window
-- (their start → this month) regardless of the display range — the other
-- sources (cash, GL, custody, owned-bank) are single scans and stay unbounded.
...
begin
  ...
  months as (
    -- bounded: never before the partner's start (or 24 months back if unset),
    -- never past the current month — no matter how wide the display range is.
