-- 0181: fully-loaded client statements — regional and head-office cost carried
-- down to the individual client, on either basis.
--
-- The client statement charged a client only what was booked directly against
-- it. In June 2026 that was PKR 66,105 of expenses against PKR 1,499,937 that
-- named no client at all: 96% of what the business spent was invisible on the
-- report that is supposed to say whether a client is worth having.
--
-- Costs now flow the whole way down, in the same order the partner allocation
-- uses:
--
--   1. direct        payroll split by days worked + expenses booked to the client
--   2. regional      the region's OWN unattributed cost, spread across its
--                    clients pro-rata by revenue
--   3. head office   head office's cost, spread across every client pro-rata by
--                    revenue
--
-- Step 3 is deliberately one hop rather than two. Apportioning head office to
-- regions by revenue and then from a region to its clients by revenue is
-- algebraically identical to apportioning it straight to clients by revenue:
--
--   ho x (rev_R / rev_total) x (rev_c / rev_R)  =  ho x (rev_c / rev_total)
--
-- so the extra hop would only add rounding. The two-step version is still what
-- the Partnership Report shows, because there the region is the thing being
-- reported on.
--
-- Every cost is either direct or apportioned, so the client rows sum back to
-- company profit. The exceptions are honest ones and are left visible rather
-- than smeared: a region holding overhead but no clients has nowhere to put it,
-- and a region whose clients billed nothing falls back to an equal split
-- because a revenue-weighted one would be a division by zero.
create or replace function public.client_statement_loaded(
  p_start date,
  p_end   date,
...
returns table(
  ...
  region_name       text,
  revenue           numeric,
  direct_payroll    numeric,
  direct_expenses   numeric,
  regional_overhead numeric,
  ho_share          numeric,
  net               numeric
)
language sql
...
as $function$
  ...
  -- ---- revenue per client: invoiced, or received ----
  ip as (
    -- invoice_payments.client_id is a denormalisation that is mostly NOT
    -- populated: 24 of 27 rows in June 2026 carry only invoice_id, so grouping
    -- on client_id alone attributed 5% of cash received and dropped PKR 12.1m
    -- on the floor. Falling back through the invoice is what puts cash revenue
    -- on the client that actually earned it.
  ...
  iv as (
    select i.client_id, sum(i.invoice_amount)::numeric as amt
     ...
     where i.company_id = cid.company_id and i.invoice_date between p_start and p_end
  ...
  -- ---- payroll, split across the clients each guard actually worked for ----
  paid as (
  ...
  ps as (
    select employee_id, period_month, fallback_client, emp_branch, sum(salary) as salary
      from paid
     where counts and eff_date between p_start and p_end
     group by 1, 2, 3, 4
  ),
  days as (
  ...
  pay_split as (
    ...
    union all
    select ps.fallback_client, ps.emp_branch, ps.salary
      from ps
     where not exists (
  ...
  pay_client as (
    select client_id, sum(cost) as amt from pay_split where client_id is not null group by 1
  ),
  -- Payroll that named no client is overhead, attributed to the region the
  -- EMPLOYEE sits in (office staff, unposted guards).
  pay_overhead as (
    select emp_branch as b, sum(cost) as amt from pay_split where client_id is null group by 1
  ),
  -- ---- expenses ----
  exp_rows as (
  ...
  -- ---- overhead per region, split into head office and the rest ----
  overhead as (
    select b, sum(amt) as amt from (
      select b, amt from pay_overhead
      union all
      select b, amt from exp_overhead
    ) u group by 1
  ),
  ho as (
    select coalesce(sum(o.amt), 0) as pool
      from overhead o join public.branches br on br.id = o.b
     where br.is_head_office
  ),
  region_overhead as (
    select o.b, o.amt
      from overhead o
      left join public.branches br on br.id = o.b
  ...
  rev_region as (
    select cl.branch_id as b, coalesce(sum(rev.amt), 0) as amt, count(*) as n_clients
      from cl join rev on rev.client_id = cl.id
     group by cl.branch_id
  )
  select
    cl.id,
    cl.name,
    ...
    -- Region's own overhead, by revenue share; equal split when the region
    -- billed nothing, since a revenue-weighted split would divide by zero.
    round(
      coalesce(ro.amt, 0) * case
        when coalesce(rr.amt, 0) > 0 then coalesce(rev.amt, 0) / rr.amt
        when coalesce(rr.n_clients, 0) > 0 then 1.0 / rr.n_clients
        else 0
      end, 2),
    -- Head office, straight to the client by revenue share (see note above).
    round(
      ho.pool * case when rt.amt > 0 then coalesce(rev.amt, 0) / rt.amt else 0 end, 2),
    round(
      coalesce(rev.amt, 0)
      - coalesce(pay_client.amt, 0)
      - coalesce(exp_client.amt, 0)
      - coalesce(ro.amt, 0) * case
          when coalesce(rr.amt, 0) > 0 then coalesce(rev.amt, 0) / rr.amt
          when coalesce(rr.n_clients, 0) > 0 then 1.0 / rr.n_clients
          else 0
        end
      - ho.pool * case when rt.amt > 0 then coalesce(rev.amt, 0) / rt.amt else 0 end
    , 2)
  from cl
  cross join ho
...
revoke execute on function public.client_statement_loaded(date, date, text) from public, anon;
grant execute on function public.client_statement_loaded(date, date, text) to authenticated;
