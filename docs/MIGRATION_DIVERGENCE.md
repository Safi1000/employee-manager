...
**Regated on 2026-09-03: production alone.** `crm-design-dev` no longer exists,
so two of the four demanded credentials named a database that cannot answer and
the gate was unsatisfiable by construction. It now requires
`SUPABASE_PROD_URL` / `SUPABASE_PROD_SERVICE_ROLE_KEY` and **still fails hard**
when they are absent — the silent skip is not coming back. Note what this does
and does not fix: it makes the hook satisfiable, so the `--no-verify` habit has
somewhere else to go. It does nothing about defects 1 and 3, which are about the
*event*, not the threshold. The real fix is the nightly database-side check.
